...
const char * const BITCOIN_CONF_FILENAME = "educoin.conf";
const char * const BITCOIN_PID_FILENAME = "educoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "educoin" is a composite category enabling all Educoin-related debug output
                if(ptrCategory->count(std::string("educoin"))) {
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Educoin";
#else
    // Unix
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty educoin.conf if it does not excist
