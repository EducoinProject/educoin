...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "educoin.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for educoind to start. This means that RPC is accessible and fully initialized.
    Raise an exception if educoind exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('educoind exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        # Create cache directories, run educoinds:
        for i in range(MAX_NODES):
            datadir=initialize_datadir(cachedir, i)
            args = [ os.getenv("EDUD", "educoind"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            if os.getenv("PYTHON_DEBUG", ""):
                print("initialize_chain: educoind started, waiting for RPC to come up")
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start a educoind and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("EDUD", "educoind")
    ...
    bitcoind_processes[i] = subprocess.Popen(args, stderr=stderr)
    if os.getenv("PYTHON_DEBUG", ""):
        print("start_node: educoind started, waiting for RPC to come up")
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start multiple educoinds, return RPC connections to them
    """
